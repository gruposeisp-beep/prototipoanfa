...
class FutureAddMatchActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_future_add_match)
        supportActionBar?.title = "Añadir Partido Futuro"
    ...
    private fun fetchTablesForSpinner() {
        database.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                val adapter = ArrayAdapter(this@FutureAddMatchActivity, android.R.layout.simple_spinner_item, tableTitles)
                adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                spinnerTables.adapter = adapter
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@FutureAddMatchActivity, "Error al cargar tablas: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
